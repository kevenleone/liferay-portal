...
export default ({childrenContext}) => {
	...
	return (fieldSetName) => {
		let {dataDefinitionFields} = childrenState.dataDefinition;
		dataDefinitionFields = dataDefinitionFields.filter(
			({fieldType}) => fieldType !== 'fieldset'
		);
		const dataLayoutRows = dataDefinitionFields.map((ddFields) => {
			return {
				dataLayoutColumns: [
					{columnSize: 12, fieldNames: [ddFields.name]},
				],
			};
		});
		...
		const fieldSetDefinition = {
			...
			defaultDataLayout: {
				...
				name: {
					[defaultLanguageId]: `${fieldSetName}Layout`,
				},
			},
			name: {
