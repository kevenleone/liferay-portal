...
const DisplayTickets = ({ tickets }) => {
  if (!tickets.length) {
    return defaultTickets.map((defaultTicket, index) => (
      ...
      {tickets.map((recentTicket, index) => (
        <li className="pb-2" key={index}>
          <span>
            ...
            }" for support region ${recentTicket.region} ${relativeTime.from(
              recentTicket.dateCreated
            )}. `}
          </span>

          {recentTicket.suggestions && !!recentTicket.suggestions.length && (
            <div className="m-2 p-2">
              <em>Update:</em> Here are some suggestions for resources re: this
              ticket:&nbsp;
              {recentTicket.suggestions.map((suggestion, index) => (
                <span key={index}>
                  <a
